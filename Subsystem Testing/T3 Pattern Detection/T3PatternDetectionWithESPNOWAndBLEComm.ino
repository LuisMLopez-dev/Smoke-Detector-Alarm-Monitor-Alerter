/*
  TEST: T3 Detection + ESP-NOW Transmission + BLE Notification

  PURPOSE:
  Detect Temporal-3 smoke alarm pattern, then:
  - Transmit alarm state to ESP-NOW receivers (edge + heartbeat)
  - Notify connected phone via BLE (edge + heartbeat)

  METHOD:
  - Envelope follower + hysteresis + FSM for T3 detection
  - Alarm latch (8s hold)
  - ESP-NOW: edge send on state change + 1 Hz heartbeat
...
void onSent(const wifi_tx_info_t *info, esp_now_send_status_t status) {
  // Uncomment for per-packet status (noisy)
  // Serial.print("ESP-NOW: ");
  // Serial.println(status == ESP_NOW_SEND_SUCCESS ? "Success" : "Fail");
}

// ============================================================
// UNIFIED TRANSMIT — ESP-NOW + BLE
// ============================================================
...
void loop() {
  ...
  if (nowMicros - lastSampleTime >= SAMPLE_PERIOD_US) {
    ...
    if (currentState) {
      if (envelope < THRESHOLD_LOW) currentState = false;
    } else {
      if (envelope > THRESHOLD_HIGH) currentState = true;
    }
    ...
    if (currentState) {
      if (now - stateStartTime > PULSE_MAX + 200) pulseCount = 0;
    } else {
      if (pulseCount > 0 && pulseCount < 3) {
        if (now - stateStartTime > SHORT_PAUSE_MAX + 150) pulseCount = 0;
      } else if (pulseCount == 3) {
        if (now - stateStartTime > LONG_PAUSE_MAX + 200) pulseCount = 0;
      }
    }
